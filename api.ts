...
async function extract_table(
	...
	for (let i = 0; i < rows.length; i++) {
		...
		for (let j = 0; j < cells.length; j++) {
			...
			const selector = `${table_selector} tr:nth-child(${i + 1}) td:nth-child(${
				j + 1
			})`;
...
async function enter_location(
	...
	let plz_lst_index = 0;
	while (
		(await get_field_value(page, "Postleitzahl", outer)) !== location.plz
	) {
				...
				if (
					...
					try {
						...
						if (
							...
							await field_input(page, "Postleitzahl", location.plz, outer);
						}
					}
					break;
				}
		...
		try {
			...
			const obj = Object.fromEntries(
				table.map((row) => [row?.[3]?.[0], row?.[3]?.[1]] as [string, string]),
			);
			await page.click(obj[location.plz] ?? panic("PLZ not in table"));
			await close_table_popup(page);
			await sleep(200);
			...
			if (
				!(e instanceof Error && e.message.includes("timeout on extract_table"))
			) {
				throw e;
			}
		}
...
async function handle_address_section(
	...
	if (form.payback_number) {
		await field_input(
			...
			form.payback_number.replaceAll(" ", ""),
		);
	}
}
...
async function add_optional_packages(
	...
	await ccheck(
		page,
		"input[aria-label='UHD-Sender']",
		option_names.includes("uhd"),
	);
...
async function handle_contract_section(
	...
	await sleep(2000);
	{
		...
		await custom_field_input(
			page,
			"td.siebui-popup-filter > span.siebui-popup-button > input.siebui-ctrl-input",
			process.env.SIEBEL_NAME ?? panic("SIEBEL_NAME not set"),
		);
		await close_table_popup(page);
		await field_input(
			page,
			"Gutscheinnummer",
			process.env.SIEBEL_KEY ?? panic("SIEBEL_KEY not set"),
		);
	}
	{
		// Angebot
...
async function handle_customer_section(page: Page, form: SkyFormData) {
	...
	const telep = form.telefon.startsWith("49")
		? "0" + form.telefon.slice(2)
		: form.telefon;
...
async function handle_tech_section(page: Page, form: SkyFormData) {
	...
	// 	const outer =
	// 		'div[title="Abweichende Lieferadresse / DHL Packstation Formularapplet"]';
	// 	await enter_location(
	// 		page,
	// 		{
	// 			ort: form.ort_liefer,
	...
	// 		outer,
	// 	);
	// 	await field_input(page, "Vorname", form.vorname, outer);
	// 	await field_input(page, "Name", form.nachname, outer);
	// 	await field_input(
	// 		page,
...
async function handle_overview_section(page: Page, form: SkyFormData) {
	...
	{
		const actual_str = await page.inputValue('input[aria-label="Abogebühren"]');
	...
	{
		const table = await extract_table(page, 'table[summary="Service"] tbody');
