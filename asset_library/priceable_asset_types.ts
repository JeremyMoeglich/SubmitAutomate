...
export interface Priceable_Asset<
    T extends priceable_asset_id = priceable_asset_id
> extends Asset<T> {
    price: Price
    selector: selector
}

export interface Dynamic_Priceable_Asset<T extends priceable_asset_id>
    extends Asset<T> {
    price: number | Price
    selector?: selector
}
...
export function apply_to_price<T extends priceable_asset_id>(
    ...
    if (timeframe === 'all') {
        asset.price = Object.fromEntries(
            Object.entries(asset.price).map((key, value) => [
                key,
                value * factor,
            ])
        )
