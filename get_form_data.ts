...
function to_form_data(text: string): SkyFormData {
	...
	const alt = <T extends keyof typeof obj>(opt: T[]) => {
		return opt.map((e) => obj[e]).reduce((a, b) => a ?? b, undefined);
	};
	...
	const clite =
		...
		(v: unknown) => {
			...
			const similarities = lst.map(
				(l, i) =>
					[i, compareTwoStrings(val.toLowerCase(), l.toLowerCase())] as [
						number,
						number,
					],
			);
	...
	const sepa_vorhanden =
		lite(
			["Ja", "Nein"],
			"sepa_present",
		)(obj["SEPA Bankinformationen vorhanden?"]) === "Ja"
			? true
			: false;
	...
	const [kontoinhaber, kontoinhaber_info] = (() => {
		...
		const kontoinhaber =
			kontoinhaber_text === "Abonnent ist Kontoinhaber" ? true : false;
	...
	const base_string = strp(
		obj["In Ihrem Paket inklusive:"],
		"base_package_string",
	);
	...
	const zubuchoptionen: zubuchoption_id[] = (() => {
		...
		const zubuchoptionen_table: Record<string, zubuchoption_id | undefined> = {
			...
			"UHD 12 Monate für die gebuchten Pakete GRATIS, danach zzgl. ? 5 mtl.":
				"uhd",
			...
			"Sky Go Plus (auf 3 Geräten gleichzeitig streamen und herunterladen) --> ? 5 mtl.":
				"skygoplus",
			"Sky Kids --> ? 5 mtl.": "kids",
			"Sky Q + 1 Stream mit der Sky Go App (automatisch inklusive)": undefined,
		};
			...
			? {
					...
					iban: (() => {
						...
						const codes = {
							...
							[ValidationErrorsIBAN.ChecksumNotNumber]: "Checksum not number",
						...
						console.log(
							`Invalid IBAN: ${validation_result.errorCodes
								.map((e) => codes[e])
								.join(", ")}`,
						);
